...
class KinematicModel_3:
    ...
    def get_front_state(self):
        ...
        return front_x, front_y, self.psi, self.v
...
def stanley_control(robot_state, refer_path, refer_path_psi, k):
    """stanley控制
    ...
    # 2. 判断e_y的正负, 即判断前轮中心点是否在参考线切线的下方
    # if (robot_state[1] - current_ref_point[1]) * math.cos(psi_t) - (
    #     robot_state[0] - current_ref_point[0]) * math.sin(psi_t) <= 0:
    if (current_ref_point[1] - robot_state[1]) * math.cos(psi_t) - (
        current_ref_point[0] - robot_state[0]) * math.sin(psi_t) >= 0:
...
def main():
    ...
    refer_path[:, 1] = 2 * np.sin(refer_path[:, 0] / 3.0) + 2.5 * np.cos(
        refer_path[:, 0] / 2.0
    )  # 生成正弦轨迹
    refer_path_psi = [
        math.atan2(
            refer_path[i + 1, 1] - refer_path[i, 1],
    ...
    # plt.ylim([-3,3])
    for _ in range(500):
        front_x, front_y, psi, v = ugv.get_front_state()
        ...
        delta, ind = stanley_control(robot_state, refer_path, refer_path_psi, k)
